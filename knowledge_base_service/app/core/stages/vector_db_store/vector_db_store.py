...
from app.domain.models.vector import CodeVectorRecord
...
class VectorDBStoreStage(PipelineStageHandler):
    ...
    async def execute(self, context: PipelineContext) -> StageResult:
        ...
        try:
            ...
            node_type_configs = [
                ("File", "file_vectors"),
                ("Class", "class_vectors"),
                ("Method", "method_vectors"),
                ("Module", "semantic_vectors"),
                ("Workflow", "semantic_vectors"),
            ]

            # 顺序处理各类型节点（降低并发复杂度，控制内存使用）
            for node_type, stat_key in node_type_configs:
    ...
    async def _process_node_type_in_batches(
        ...
        while skip < total:
            ...
            if records:
                await vector_db.insert(VECTOR_COLLECTION_NAME, records)
    ...
    def _convert_nodes_to_contents(
        ...
        Returns:
            内容列表，每项包含 type, id, name, summary
        """
        contents = []
